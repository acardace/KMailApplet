...
const string MailWatcher::NO_MAIL_ICON_PATH =
    "/usr/share/kmailapplet/images/mail.png";
const string MailWatcher::NEW_MAIL_ICON_PATH =
    "/usr/share/kmailapplet/images/newmail.png";
string mailToWatch;

MailWatcher::MailWatcher(shared_ptr<QSystemTrayIcon> sysTrayIcon) {
  connect(this, &MailWatcher::directoryChanged, this,
          &MailWatcher::slotDirectoryChanged);
...
bool MailWatcher::slotDirectoryChanged(const QString &path) {
  ...
  if (fs::is_empty(pathToWatch)) {
    ...
    sysTrayIcon->showMessage(MailWatcher::SYSTRAY_NAME.c_str(), msgText.c_str(),
...
bool MailWatcher::checkMails() {
  ...
  for (vector<fs::path>::iterator iter = mailsToWatch.begin();
       iter != mailsToWatch.end(); ++iter) {
...
bool MailWatcher::isMailDir(fs::path &p) {
  bool cur_dir = false;
  bool new_dir = false;
  for (fs::directory_iterator iter(p); iter != fs::directory_iterator();
       ++iter) {
